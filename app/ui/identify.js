 ...
 * identify(video) -> Promise<{sc, tc, fontsDir, chapters}>（失败项返回空串，不抛错） */
async function identify(video) {
  ...
  return { sc: m.sc || '', tc: m.tc || '', fontsDir: fd.fonts_dir || '', chapters: ch.chapters || '' };
}
/* 便捷：把识别结果填入输入框（已有值不覆盖） */
function applyIdentify(inputSc, inputTc, inputFonts, id, inputChapters) {
  let changed = false;
